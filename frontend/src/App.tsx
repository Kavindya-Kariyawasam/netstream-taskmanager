...
import FileUpload from "./components/FileUpload"; // ✅ added
import { useState } from "react";

function App() {
  const [showUpload, setShowUpload] = useState(true); // optional toggle
  ...
  return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 flex flex-col">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                <button
                    ...
                    className="px-3 py-1 text-sm font-medium text-indigo-600 hover:text-indigo-800 bg-indigo-50 rounded-md border border-indigo-200 transition"
                >
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Right: File Upload Sidebar */}
          {showUpload && (
              <aside className="bg-white/70 backdrop-blur-sm shadow-sm rounded-2xl border border-slate-200 p-6">
